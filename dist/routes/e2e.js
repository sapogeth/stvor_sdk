...
async function generateBotIdentity() {
    const keyPair = await webcrypto.subtle.generateKey({ name: 'ECDH', namedCurve: 'P-256' }, true, ['deriveKey', 'deriveBits']);
    const pubJwk = await webcrypto.subtle.exportKey('jwk', keyPair.publicKey);
    botIdentity = {
        publicKey: pubJwk,
        privateKey: keyPair.privateKey,
    };
    users.set('bot', { publicKey: pubJwk, privateKey: keyPair.privateKey });
}
// Error response factory
function errorResponse(reply, error, message) {
    return reply.status(400).send({ error, message });
}
export default async function e2eRoutes(app) {
    if (!users.has('bot'))
        await generateBotIdentity();
    ...
    app.post('/register', async (req, reply) => {
        ...
        // New ratchet format
        if (publicKeys) {
            if (typeof publicKeys.identityKey !== 'string' ||
    ...
    app.get('/public-key/:user_id', async (req, reply) => {
        ...
        if (!user)
            return reply.code(404).send({ error: 'NOT_FOUND', message: `User '${user_id}' not found` });
