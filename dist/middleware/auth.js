...
export function authError(reply, code, message) {
...
export async function authMiddleware(request, reply) {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return authError(reply, 'API_KEY_REQUIRED', 'API key is required to access this endpoint. Include "Authorization: Bearer <API_KEY>" header.');
    }
    const apiKey = authHeader.substring(7); // Remove 'Bearer ' prefix
    // Validate API key
    const projectId = getProjectIdByApiKey(apiKey);
    if (!projectId) {
        return authError(reply, 'API_KEY_INVALID', 'Invalid or revoked API key. Check your credentials or generate a new key.');
    }
...
export function getProjectId(request) {
