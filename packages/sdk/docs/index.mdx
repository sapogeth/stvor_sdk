...
STVOR implements X3DH key exchange and Double Ratchet encryption (the same cryptography used by Signal) so you can add world-class end-to-end encryption without being a cryptographer.
...
❌ **Post-quantum security** — Uses classical X25519 (planned: ML-KEM hybrid)  
❌ **Persistent storage** — Keys/sessions stored in-memory (v2.1)  
...
|---------|--------|-------|
| **Core E2EE** | ✅ Production | X3DH + Double Ratchet fully implemented |
...
Requires distributed state (Redis/PostgreSQL). Coming in v2.2.
...
- In-memory storage (v2.1) — keys cleared on restart
- Single-instance ready — multi-instance needs v2.2
...
*STVOR SDK v2.4 — Secure by default, honest by design. (Feb 2026)*
