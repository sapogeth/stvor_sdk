...
interface UserEntry {
  publicKeys: unknown;
  messages: Array<{ id: string; from: string; ciphertext: string; header: string; timestamp: string }>;
  lastActivity: number;
}

// projectId → userId → UserEntry
const registry = new Map<string, Map<string, UserEntry>>();
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && path === '/register') {
    ...
    if (!user_id || typeof user_id !== 'string') return send(res, 400, { error: 'Invalid user_id' });
  ...
  if (req.method === 'POST' && path === '/message') {
    ...
    if (to === from) return send(res, 400, { error: 'Cannot send message to yourself' });
  ...
  if (req.method === 'DELETE' && delMatch) {
    ...
    if (project) {
      for (const user of project.values()) {
        ...
        if (idx !== -1) { user.messages.splice(idx, 1); return send(res, 200, { status: 'deleted' }); }
      }
    }
  ...
  if (req.method === 'GET' && path === '/stats') {
    ...
    for (const project of registry.values()) {
      for (const user of project.values()) { users++; pending += user.messages.length; }
    }
    return send(res, 200, {
      status: 'ok',
