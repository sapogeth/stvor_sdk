...
function ab(buf: BufferSource): ArrayBuffer {
  if (buf instanceof ArrayBuffer) return buf;
  const v = buf as ArrayBufferView;
  const b = v.buffer;
  if (b instanceof ArrayBuffer) return b.slice(v.byteOffset, v.byteOffset + v.byteLength);
  // SharedArrayBuffer — copy to plain ArrayBuffer
  const copy = new Uint8Array(v.byteLength);
  copy.set(new Uint8Array(b as unknown as ArrayBuffer, v.byteOffset, v.byteLength));
  return copy.buffer;
}
...
function u32be(n: number): ArrayBuffer {
...
export interface WebKeyPair {
  ...
  publicRaw:  ArrayBuffer; // 65 bytes, uncompressed P-256
}
...
export interface WebSessionState {
  ...
  skippedKeys: Map<string, ArrayBuffer>; // "hexPub:msgNum" → messageKey
...
export interface WebSerializedPublicKeys {
  identityKey:           string; // base64url, 65-byte raw P-256
  signedPreKey:          string;
  signedPreKeySignature: string; // base64url ECDSA-P256-SHA256 DER
  oneTimePreKey:         string;
}

// ─── Низкоуровневые операции ──────────────────────────────────────────────────

async function generateECDH(): Promise<WebKeyPair> {
...
async function generateECDSA(): Promise<{ publicKey: CryptoKey; privateKey: CryptoKey; publicRaw: ArrayBuffer }> {
  const kp = await subtle.generateKey(
...
async function ecdsaSign(data: ArrayBuffer, privateKey: CryptoKey): Promise<ArrayBuffer> {
...
async function ecdhSecret(myPrivate: CryptoKey, theirPublicRaw: ArrayBuffer): Promise<ArrayBuffer> {
...
async function kdfRK(rk: ArrayBuffer, dhOut: ArrayBuffer): Promise<{ rootKey: ArrayBuffer; chainKey: ArrayBuffer }> {
...
async function kdfCK(ck: ArrayBuffer): Promise<{ chainKey: ArrayBuffer; messageKey: ArrayBuffer }> {
...
async function aeadEncrypt(
  ...
  const nonce      = nonceBytes.buffer.slice(nonceBytes.byteOffset, nonceBytes.byteOffset + 12) as ArrayBuffer;
...
function buildHeader(pub: ArrayBuffer, prevSendCount: number, msgNum: number, nonce: Uint8Array): ArrayBuffer {
...
function parseHeader(h: ArrayBuffer): { pub: ArrayBuffer; prevSendCount: number; msgNum: number; nonce: ArrayBuffer } {
  return {
    pub:           h.slice(0, 65),
...
async function x3dhSymmetric(
  myIK: WebKeyPair,
  mySPK: WebKeyPair,
  peerIKRaw: ArrayBuffer,
  peerSPKRaw: ArrayBuffer,
): Promise<ArrayBuffer> {
...
async function dhRatchetStep(s: WebSessionState, theirNewKeyRaw: ArrayBuffer): Promise<void> {
...
async function skipKeys(s: WebSessionState, until: number): Promise<void> {
...
export async function webEncrypt(
  ...
  const { ciphertext, nonce } = await aeadEncrypt(
    messageKey,
    plaintext,
    session.myRatchetPair.publicRaw,
  );

  const header = buildHeader(
    session.myRatchetPair.publicRaw,
    session.prevSendCount,
    session.sendCount,
    nonce,
  );
...
export async function webDecrypt(
  ...
  if (curPub !== theirPubB64) {
    if (session.theirRatchetPublicRaw) await skipKeys(session, prevSendCount);
...
export interface WebIdentityKeys {
  ...
  ikEcdsaPair: { publicKey: CryptoKey; privateKey: CryptoKey; publicRaw: ArrayBuffer };
...
export async function generateWebIdentityKeys(): Promise<WebIdentityKeys> {
...
export async function verifyWebSPK(
  spkRaw: ArrayBuffer,
  sigRaw: ArrayBuffer,
  ikEcdsaPublicRaw: ArrayBuffer,
): Promise<boolean> {
...
export async function serializeWebSession(s: WebSessionState): Promise<WebSessionSerialized> {
  const mrPrivRaw = await subtle.exportKey('pkcs8', s.myRatchetPair.privateKey);
  const skipped: Record<string, string> = {};
  for (const [k, v] of s.skippedKeys) skipped[k] = b64enc(v);
...
async function importECDHPair(pubRaw: ArrayBuffer, privPkcs8: ArrayBuffer): Promise<WebKeyPair> {
...
export async function deserializeWebSession(d: WebSessionSerialized): Promise<WebSessionState> {
  const mrPubRaw  = b64dec(d.mrPub);
  const mrPrivRaw = b64dec(d.mrPriv);
  const mrPair    = await importECDHPair(mrPubRaw, mrPrivRaw);

  let mySPKPair: WebKeyPair | null = null;
  if (d.mySPKPub && d.mySPKPriv) {
    mySPKPair = await importECDHPair(b64dec(d.mySPKPub), b64dec(d.mySPKPriv));
  }

  const skipped = new Map<string, ArrayBuffer>();
  for (const [k, v] of Object.entries(d.skipped)) skipped.set(k, b64dec(v));
