...
export class StvorFacadeClient {
  private crypto: CryptoSession;
  private handlers: MessageHandler[] = [];
  private userAvailableHandlers: UserAvailableHandler[] = [];
  private knownPubKeys: Map<UserId, string> = new Map();
  private pendingKeyResolvers: Map<UserId, Array<() => void>> = new Map();

  constructor(
    public readonly userId: UserId,
    private readonly relay: RelayClient,
    private readonly defaultTimeout: number = DEFAULT_RECIPIENT_TIMEOUT
  ) {
  ...
  private async handleRelayMessage(m: any) {
    ...
    if (m.type === 'message' && m.to === this.userId && m.payload) {
      const payload: EncryptedMessage = m.payload;
      const sender = m.from as UserId;
      try {
        const plain = this.crypto.decrypt(payload, payload.senderPub);
        const text = new TextDecoder().decode(plain);
        for (const h of this.handlers) h(sender, text);
      } catch (e) {
   ...
   * ```
   */
  async waitForUser(userId: UserId, timeoutMs: number = this.defaultTimeout): Promise<void> {
    // Already available
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        if (resolvers) {
          ...
          if (resolvers.length === 0) this.pendingKeyResolvers.delete(userId);
        }
        
        reject(new StvorError(
          Errors.RECIPIENT_TIMEOUT,
          `Timed out waiting for user "${userId}" after ${timeoutMs}ms. ` +
          `The user may not be connected to the relay. ` +
          `Ensure both parties are online before sending messages.`,
          'Verify the recipient is connected, or increase timeout',
          true
        ));
      }, timeoutMs);
  ...
  async send(
    ...
    if (!recipientId || typeof recipientId !== 'string' || recipientId.trim().length === 0) {
      throw new StvorError(
        ...
        'Provide a valid user ID for the recipient'
      );
    }
    ...
    if (recipientId === this.userId) {
      throw new StvorError(
        ...
        'Provide a different recipient ID'
      );
    }
    ...
    if (!recipientPub) {
      if (!waitForRecipient) {
        throw new StvorError(
          ...
          `Recipient "${recipientId}" is not available. ` +
          `Their public key has not been announced to the relay. ` +
          `Use waitForUser() or enable waitForRecipient option.`,
          'Call waitForUser(recipientId) before sending, or ensure recipient is connected',
          false
        );
      }
      ...
      if (!recipientPub) {
        ...
        throw new StvorError(
          Errors.RECIPIENT_NOT_FOUND, 
          `Recipient "${recipientId}" key resolution failed unexpectedly.`,
    ...
    try {
      ...
      const payload = this.crypto.encrypt(plain as Uint8Array, recipientPub);
      const msg = { type: 'message', to: recipientId, from: this.userId, payload };
      this.relay.send(msg);
    } catch (e: any) {
      throw new StvorError(
        Errors.ENCRYPTION_FAILED,
        `Failed to encrypt message: ${e?.message || 'unknown error'}`,
        'Ensure recipient public key is valid and your key is not corrupted'
      );
    }
  }
  ...
  onUserAvailable(handler: UserAvailableHandler): () => void {
    ...
    return () => {
      ...
      if (i >= 0) this.userAvailableHandlers.splice(i, 1);
    };
  }
  ...
  async encryptMessage(
    ...
    if (recipientId === this.userId) {
      throw new StvorError(
        Errors.INVALID_INPUT,
        'Cannot encrypt message to yourself'
      );
    }
    ...
    if (!recipientPub) {
      throw new StvorError(
        Errors.RECIPIENT_NOT_FOUND,
        `Recipient "${recipientId}" public key not available. Call waitForUser() first.`
      );
    }
   ...
   * ```
   */
  async decryptMessage(encrypted: EncryptedMessage): Promise<MessageContent> {
    ...
    try {
      ...
      throw new StvorError(
        Errors.MESSAGE_INTEGRITY_FAILED,
        `Failed to decrypt message: ${e?.message || 'authentication tag verification failed'}`
      );
    }
  }
  ...
  addPeerKey(userId: UserId, publicKeyBase64: string): void {
    ...
    if (!publicKeyBase64 || typeof publicKeyBase64 !== 'string') {
      throw new StvorError(
        Errors.INVALID_INPUT,
        'Public key must be a base64 string'
      );
    }
...
export class StvorApp {
  private clients = new Map<UserId, StvorFacadeClient>();

  constructor(private readonly config: StvorAppConfig) {}
  ...
  async connect(userId: UserId): Promise<StvorFacadeClient> {
    ...
    const relay = new RelayClient(
      this.config.relayUrl ?? 'wss://stvor.xyz/relay',
