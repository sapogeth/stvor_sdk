...
import {
  WebKeyPair,
  WebSessionState,
  WebSerializedPublicKeys,
  WebIdentityKeys,
  generateWebIdentityKeys,
  webEstablishSession,
  webEncrypt,
  webDecrypt,
  serializeWebSession,
  deserializeWebSession,
  verifyWebSPK,
} from './web-ratchet.js';
...
class HttpRelay {
  ...
  async register(userId: string, publicKeys: WebSerializedPublicKeys): Promise<void> {
  ...
  async getPublicKeys(userId: string): Promise<WebSerializedPublicKeys | null> {
    const res = await fetch(`${this.url}/public-key/${encodeURIComponent(userId)}`, {
      headers: this.headers(),
    });
    if (res.status === 404) return null;
    if (!res.ok) throw new Error(`getPublicKeys failed: ${res.status}`);
    const d = await res.json();
    return d.publicKeys as WebSerializedPublicKeys;
  }

  async sendMessage(to: string, from: string, ciphertext: string, header: string): Promise<void> {
  ...
  async fetchMessages(userId: string): Promise<Array<{
...
export class StvorWebSDK {
  ...
  private readonly relay: HttpRelay;
  private readonly store: IDBStore;

  private identity: WebIdentityKeys | null = null;
  private sessions = new Map<string, WebSessionState>();
  private handlers = new Set<MessageHandler>();
  private pollTimer: ReturnType<typeof setTimeout> | null = null;
  private alive = true;
  private readonly pollIntervalMs: number;

  private constructor(opts: WebSDKOptions) {
  ...
  static async create(opts: WebSDKOptions): Promise<StvorWebSDK> {
    if (!opts.appToken?.startsWith('stvor_')) throw new Error('Invalid appToken');
    if (!opts.userId || !opts.relayUrl) throw new Error('userId and relayUrl are required');
  ...
  private async _init(): Promise<void> {
    ...
    try { await this.relay.healthCheck(); }
  ...
  private async _loadOrGenerateIdentity(): Promise<void> {
    const stored = await this.store.get<{
      ikEcdsaPub: string; ikEcdsaPriv: string;
      ikEcdhPub: string;  ikEcdhPriv: string;
      spkPub: string;     spkPriv: string;
      spkSig: string;
    }>('identity');
    ...
    if (stored) {
      ...
      const importECDH = async (pubB64: string, privB64: string): Promise<WebKeyPair> => {
      ...
      const importECDSA = async (pubB64: string, privB64: string) => {
        const pubRaw  = b642ab(pubB64);
        const privRaw = b642ab(privB64);
        const [pub, priv] = await Promise.all([
  ...
  private async _persistIdentity(): Promise<void> {
    const subtle = globalThis.crypto.subtle;
  ...
  private _serializePublicKeys(): WebSerializedPublicKeys {
    const id = this.identity!;
    return {
  ...
  private async _loadSessions(): Promise<void> {
    const keys = await this.store.get<string[]>('session-keys') ?? [];
    for (const peerId of keys) {
      const raw = await this.store.get<object>(`session-${peerId}`);
      if (raw) {
        try {
          const session = await deserializeWebSession(raw as any);
  ...
  private async _saveSession(peerId: string, session: WebSessionState): Promise<void> {
  ...
  private async _getOrEstablishSession(peerId: string): Promise<WebSessionState> {
    if (this.sessions.has(peerId)) return this.sessions.get(peerId)!;

    const peerKeys = await this.relay.getPublicKeys(peerId);
    if (!peerKeys) throw new Error(`Peer ${peerId} not found on relay`);
    ...
    if (!valid) throw new Error(`SPK signature invalid for ${peerId}`);

    const id = this.identity!;
    // Use ECDH key pairs for X3DH key agreement
    const session = await webEstablishSession(
      id.ikEcdhPair,
      id.spkPair,
      peerIKEcdhRaw,
      peerSPKRaw,
  ...
  async send(recipientId: string, data: unknown): Promise<void> {
    if (!this.alive) throw new Error('Client is disconnected');
    ...
    const tick = async () => {
      ...
      try {
        ...
        for (const raw of messages) {
          try { await this._processRaw(raw); }
          catch (e) { console.error('[StvorWeb] Failed to process message from', raw.from, e); }
        }
  ...
  private async _processRaw(raw: {
    ...
    for (const h of this.handlers) {
      try { await h(raw.from, decoded); }
      catch (e) { console.error('[StvorWeb] Handler error:', e); }
    }
  }
}

if (typeof window !== 'undefined') {
  (window as any).StvorWebSDK = StvorWebSDK;
}
