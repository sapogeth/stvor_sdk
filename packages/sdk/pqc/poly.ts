...
import { N, Q, modQ, ntt, invNtt, baseMul, fqmul, barrettReduce } from './ntt.js';
...
export function polyAdd(r: Poly, a: Poly, b: Poly): void {
  for (let i = 0; i < N; i++) r[i] = modQ(a[i] + b[i]);
}

export function polySub(r: Poly, a: Poly, b: Poly): void {
  for (let i = 0; i < N; i++) r[i] = modQ(a[i] - b[i] + Q);
}

export function polyNtt(a: Poly): void { ntt(a); }
export function polyInvNtt(a: Poly): void { invNtt(a); }

export function polyBaseMul(r: Poly, a: Poly, b: Poly): void {
  baseMul(r, a, b);
}
 ...
 */
export function polyCompress(a: Poly, d: number): Uint8Array {
 ...
 */
export function polyDecompress(b: Uint8Array, d: number): Poly {
  const r = polyNew();
  const mask = (1 << d) - 1;
  let pos = 0;
  let buf = 0;
 ...
 */
export function polyToBytes(a: Poly): Uint8Array {
 ...
 */
export function polyFromBytes(a: Uint8Array): Poly {
  const r = polyNew();
  for (let i = 0; i < N / 2; i++) {
 ...
 */
export function polyCBD3(seed: Uint8Array): Poly {
  const r = polyNew();
  for (let i = 0; i < N / 4; i++) {
