...
import {
  Poly, polyNew, polyAdd, polySub,
  polyNtt, polyInvNtt, polyBaseMul,
  polyCompress, polyDecompress,
  polyToBytes, polyFromBytes,
  polyCBD2, polyUniform,
} from './poly.js';
import { N, Q, modQ } from './ntt.js';
...
function pvecNew(): PolyVec {
  return Array.from({ length: K }, () => polyNew());
}

function pvecNtt(v: PolyVec): void {
  for (const p of v) polyNtt(p);
}

function pvecAdd(r: PolyVec, a: PolyVec, b: PolyVec): void {
  for (let i = 0; i < K; i++) polyAdd(r[i], a[i], b[i]);
}

function pvecDot(a: PolyVec, b: PolyVec): Poly {
  const r = polyNew();
  const tmp = polyNew();
  for (let i = 0; i < K; i++) {
...
function pvecToBytes(v: PolyVec): Uint8Array {
  const r = new Uint8Array(K * 384);
  for (let i = 0; i < K; i++) r.set(polyToBytes(v[i]), i * 384);
  return r;
}

function pvecFromBytes(b: Uint8Array): PolyVec {
  const v = pvecNew();
  for (let i = 0; i < K; i++) v[i] = polyFromBytes(b.subarray(i * 384, (i + 1) * 384));
  return v;
}

function pvecCompress(v: PolyVec, d: number): Uint8Array {
  const chunkSize = Math.ceil(N * d / 8);
  const r = new Uint8Array(K * chunkSize);
  for (let i = 0; i < K; i++) r.set(polyCompress(v[i], d), i * chunkSize);
  return r;
}

function pvecDecompress(b: Uint8Array, d: number): PolyVec {
  const chunkSize = Math.ceil(N * d / 8);
  const v = pvecNew();
  for (let i = 0; i < K; i++) {
...
function kyberEnc(
  ek: Uint8Array,
  m: Uint8Array,   // 32-byte message/seed
  r: Uint8Array,   // 32-byte randomness seed
): Uint8Array {
...
function _mlkemKeyGenInternal(d: Uint8Array, z: Uint8Array): MLKEMKeyPair {
  ...
  let nonce = 0;
  const s = pvecNew();
  for (let i = 0; i < K; i++) s[i] = polyCBD2(prf(sigma, nonce++, 64 * ETA1));
  const e = pvecNew();
  for (let i = 0; i < K; i++) e[i] = polyCBD2(prf(sigma, nonce++, 64 * ETA1));

  // Save s in normal domain for dk BEFORE NTT
  const s_normal = pvecNew();
  for (let i = 0; i < K; i++) s_normal[i].set(s[i]);
...
export function mlkemDecaps(ct: Uint8Array, dk: Uint8Array): Uint8Array {
  if (ct.length !== CT_SIZE) throw new Error(`Invalid ct size: ${ct.length}, expected ${CT_SIZE}`);
  if (dk.length !== DK_SIZE) throw new Error(`Invalid dk size: ${dk.length}`);
