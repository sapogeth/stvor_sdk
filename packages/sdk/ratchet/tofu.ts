...
export async function storeFingerprint(userId: string, fingerprint: string): Promise<void> {
  ...
  try {
    await client.query(
      'INSERT INTO fingerprints (user_id, fingerprint) VALUES ($1, $2) ON CONFLICT (user_id) DO UPDATE SET fingerprint = $2',
      [userId, fingerprint]
    );
  } finally {
    client.release();
 ...
 */
export async function verifyFingerprint(userId: string, fingerprint: string): Promise<boolean> {
  const client = await pool.connect();
  try {
...
export function handleFingerprintMismatch(userId: string): void {
