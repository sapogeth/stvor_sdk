...
export interface StvorEncodedMessage {
  type: StvorDataType;
  data: string; // Always base64url for transport
}
 ...
 */
export function encodeData(data: unknown): Buffer {
 ...
 */
export function decodeData(buffer: Buffer): unknown {
  if (buffer.length === 0) {
  ...
  switch (typeMarker) {
    ...
    case 0x06: {
      ...
      catch (e) { throw new Error(`Invalid JSON in encoded data: ${String(e)}`); }
    }
 ...
 * @returns Original data with correct type
 */
export function decodeFromBase64Url(encoded: string): unknown {
  try {
 ...
 */
export function encodeDataSafe(data: unknown): Buffer {
  try {
 ...
 */
export function decodeDataSafe(buffer: Buffer): unknown {
  try {
    return decodeData(buffer);
  } catch {
 ...
 * @returns Type string
 */
export function getEncodedDataType(buffer: Buffer): StvorDataType | 'unknown' {
  if (buffer.length === 0) return 'unknown';

  const typeMarker = buffer[0];
  const typeMap: Record<number, StvorDataType> = {
    0x01: 'string',
    0x02: 'number',
    0x03: 'boolean',
    0x04: 'null',
 ...
 */
export function calculateEncodingOverhead(data: unknown): number {
