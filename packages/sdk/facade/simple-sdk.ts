...
import { UniversalDataCodec, StvorData } from './universal-data.js';
...
export interface MessageHandler {
  (data: StvorData, metadata: { from: string; timestamp: Date }): void;
}
...
export class StvorSDK {
  private crypto: CryptoSessionManager;
  private relay: RelayClient | null;
  private lifecycle: ResourceLifecycleManager;
   ...
   */
  static async create(userId: string, options: SimpleSDKOptions = {}): Promise<StvorSDK> {
  ...
  async send(
    recipient: string,
    data: StvorData,
    options: { timeout?: number; autoEstablish?: boolean } = {},
  ): Promise<void> {
    ...
    try {
      ...
      const sendFn = async () => {
        ...
        if (!this.crypto.hasSession(recipient)) {
          ...
          if (this.verbose) console.log(`[SDK] Auto-establishing session with ${recipient}`);
          const peerKeys = await this.relay.getPublicKeys(recipient);
          if (!peerKeys) throw new Error(`Peer ${recipient} not found on relay.`);
      ...
      // Record failure
      const duration = Date.now() - startTime;
      this.circuitBreaker.recordFailure(recipient, error as Error);
  ...
  startPolling(intervalMs = 1000): void {
    if (this.pollingHandle || !this.relay) return;
    const poll = async () => {
      try {
        ...
        for (const msg of messages) {
          try {
            ...
            for (const h of [...handlers, ...wildcard]) {
              try { h(decoded, { from: msg.from, timestamp: new Date(msg.timestamp) }); }
              catch (e) { console.error('[SDK] Handler error:', e); }
            }
          } catch (e) {
   ...
   */
  async establishSession(peerId: string, peerPublicKeys: any): Promise<void> {
    if (!this.isInitialized) {
      throw new Error('SDK not initialized');
    }
  ...
  async receiveBatch(
    sender: string,
    encrypted: Array<{ ciphertext: Buffer; header: any }>,
   ...
   */
  getAnalyticsReportForRange(fromMs: number, toMs: number) {
   ...
   */
  getAnalyticsEvents(fromMs?: number, toMs?: number) {
 ...
 */
export async function createSDK(userId: string, options?: SimpleSDKOptions): Promise<StvorSDK> {
