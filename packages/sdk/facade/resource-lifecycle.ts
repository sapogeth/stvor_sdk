...
export class ResourceLifecycleManager {
  ...
  private otpManager?: OTPKeyManager;
  private options: Required<LifecycleOptions>;
  private cleanupInterval: NodeJS.Timeout | null = null;
  private sessionAccessTime: Map<string, number> = new Map();
  private isShuttingDown = false;
  private verbose: boolean;

  constructor(
    crypto: CryptoSessionManager,
    otpManager?: OTPKeyManager,
  ...
  cleanup(): void {
    ...
    try {
      ...
      if (this.verbose && (cleanedSessions > 0 || cleanedOTPs > 0)) {
        console.log(
          `[Lifecycle] Cleanup: ${cleanedSessions} sessions, ${cleanedOTPs} OTPs (${duration}ms)`,
        );
      }
    } catch (error) {
      if (this.verbose) {
  ...
  isHealthy(): boolean {
    try {
      ...
      if (stats.totalSessions > this.options.maxCachedSessions) {
        if (this.verbose) {
          console.warn(
            `[Lifecycle] Too many sessions: ${stats.totalSessions}/${this.options.maxCachedSessions}`,
          );
        }
        return false;
      }
      ...
      if (stats.totalOTPKeys > this.options.maxOTPKeys) {
        if (this.verbose) {
          console.warn(
            `[Lifecycle] Too many OTP keys: ${stats.totalOTPKeys}/${this.options.maxOTPKeys}`,
          );
        }
        return false;
      }
