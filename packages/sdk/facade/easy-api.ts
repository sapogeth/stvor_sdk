...
import { encodeData, decodeData } from './data-codec';
...
export class StvorEasyAPI {
  ...
  private cryptoManager: CryptoSessionManager;
  ...
  private constructor(
    userId: string,
    cryptoManager: CryptoSessionManager,
   ...
   * Initialize the API
   */
  static async init(config: StvorEasyAPIConfig): Promise<StvorEasyAPI> {
   ...
   */
  async send(peerId: string, data: unknown): Promise<void> {
    ...
    try {
      ...
      for (const msg of messages) {
        try {
          ...
          // Decrypt
          const plaintextB64 = this.cryptoManager.decryptFromPeer(
            msg.from,
            msg.ciphertext,
            msg.header,
          );
 ...
 * });
 */
export async function stvorInit(config: StvorEasyAPIConfig): Promise<StvorEasyAPI> {
 ...
 */
export async function quickStart(token: string, userId: string): Promise<StvorEasyAPI> {
