...
export class FileReplayStore {
  ...
  constructor(config: PersistentReplayProtection) {
    ...
    this.masterKey = crypto.pbkdf2Sync(
      ...
      Buffer.from('stvor-replay-store'),
      100000,
      32,
      'sha256'
    );
  ...
  private getReplayFile(userId: string, peerId: string): string {
  ...
  ): Promise<{ isReplay: boolean; state: any }> {
   ...
   */
  async clearReplayState(userId: string, peerId: string): Promise<void> {
    const filePath = this.getReplayFile(userId, peerId);
    try {
