...
import { KeyPair, generateKeyPair } from '../ratchet/index.js';
...
export class OTPKeyManager {
  private keyPairs: Map<number, { keyPair: KeyPair; createdAt: Date; usedAt?: Date }>;
  private nextKeyId: number = 0;
  private maxUnusedKeys: number;
  private rotationInterval: number; // milliseconds

  constructor(maxUnusedKeys: number = 100, rotationIntervalMs: number = 24 * 60 * 60 * 1000) {
   ...
   */
  private generateNewKey(): { keyId: number; keyPair: KeyPair } {
  ...
  rotateExpiredKeys(): void {
    ...
    const unusedCount = Array.from(this.keyPairs.values()).filter(
      info => !info.usedAt
    ).length;
  ...
  getStatus(): {
    ...
    usedKeys: number;
    nextKeyId: number;
    oldestUnusedAge: number; // milliseconds
  } {
    const now = Date.now();
    let unusedCount = 0;
  ...
  private toBundleFormat(keyId: number, keyPair: KeyPair): OTPKeyBundle {
    return {
      keyId,
