 ...
 */
export async function batchEncryptMessages(crypto, recipient, messages, options = {}) {
    ...
    const results = messages.map((_, i) => ({
        index: i,
        encrypted: undefined,
        error: undefined,
    }));
    ...
    for (let i = 0; i < messages.length; i += concurrency) {
        ...
        const batchPromises = batch.map(async (message, batchIndex) => {
            ...
            try {
                ...
                failed++;
                results[globalIndex].error = error;
                if (verbose) {
    ...
    const totalTime = Date.now() - startTime;
    if (verbose) {
        console.log(`[Batch] Encryption complete: ${completed} succeeded, ${failed} failed in ${totalTime}ms`);
    }
    return {
        success: failed === 0,
        results,
 ...
 * Reuses session, parallelizes operations
 */
export async function batchDecryptMessages(crypto, sender, encryptedMessages, options = {}) {
    const startTime = Date.now();
    const concurrency = options.concurrency ?? 10;
    const timeout = options.timeout ?? 30000;
    const stopOnError = options.stopOnError ?? false;
    ...
    const results = encryptedMessages.map((_, i) => ({
        index: i,
        decrypted: undefined,
        error: undefined,
    }));
    ...
    for (let i = 0; i < encryptedMessages.length; i += concurrency) {
        ...
        const batchPromises = batch.map(async (encrypted, batchIndex) => {
            const globalIndex = batchStart + batchIndex;
            try {
                const decrypted = crypto.decryptFromPeer(sender, encrypted.ciphertext, encrypted.header);
 ...
 */
export async function batchEstablishSessions(crypto, peers, options = {}) {
    const concurrency = options.concurrency ?? 10;
    ...
    for (let i = 0; i < peers.length; i += concurrency) {
        ...
        const batchPromises = batch.map(async (peer, idx) => {
            ...
            try {
                ...
                failed++;
                results[resultIndex].error = error;
                if (verbose) {
    ...
    if (verbose) {
        const totalTime = Date.now() - startTime;
        console.log(`[Batch] Established ${succeeded} sessions, ${failed} failed in ${totalTime}ms`);
    }
    return results;
}
