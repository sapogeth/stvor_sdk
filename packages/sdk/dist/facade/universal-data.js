    ...
    static encode(data) {
        try {
            ...
            if (data instanceof Map) {
                const json = JSON.stringify(Array.from(data.entries()));
    ...
    static decode(buffer) {
        try {
            ...
            switch (typeMarker) {
                ...
                case 0x02: // Number
                    if (data.length < 8)
                        throw new Error('Invalid number buffer');
                    return data.readDoubleBE(0);
                case 0x03: // Boolean
                    if (data.length < 1)
                        throw new Error('Invalid boolean buffer');
                    return data[0] === 1;
                case 0x04: // Null
                    return null;
                case 0x05: // Binary
                    if (data.length < 4)
                        throw new Error('Invalid binary buffer');
                    const length = data.readUInt32BE(0);
                    return data.slice(4, 4 + length);
                case 0x06: // Date
                    if (data.length < 8)
                        throw new Error('Invalid date buffer');
